...
Re-analysis of published summary statistics only. No genotype-level data, no
wet lab.
...
0.114–0.150. The sharpest single comparison is **Bryois pseudobulk** at 192
donors, gap **0.144**, against **PsychENCODE** bulk tissue at **1,387 donors**,
gap **0.367**.

So roughly 60% of the constrained-gene deficit is a property of **bulk tissue
RNA-seq** that single-nucleus RNA-seq doesn't share. The residual ~40% survives
every assay, resolution and sample size tested here, and is the part consistent
with selection. Full argument in
...
The genes GWAS most implicates are the genes eQTL studies are worst at finding
regulatory variants for. Two published accounts explain that observation in
incompatible ways.

**H1, the selection account** (Mostafavi, Spence, Naqvi & Pritchard, *Nature
Genetics* 2023). cis-eQTLs really are depleted near constrained genes.
Selection removes common regulatory variants of large effect at genes where
dosage matters, leaving a regulatory landscape that is real but distributed and
largely invisible to eQTL mapping. The gap is then a biological fact rather than
a measurement problem, and it should **persist at any resolution**.

**H2, the power/resolution account** (Rosen, Broadaway, Brotman, Mohlke & Love,
*AJHG* 2026; bioRxiv 2025.08.05.668745). The gap is substantially an artefact of
statistical power. Larger eQTL studies find weaker eQTLs, and those weaker
eQTLs look progressively more "GWAS-like": more distal, more
enhancer-overlapping, higher pLI. On this reading the gap should **shrink as
resolution and power increase**.

Rosen et al. frame their result as consistent with Mostafavi's signal-discovery
model rather than as a refutation of it. This project estimates *how much* of
the gap each account explains. It doesn't try to crown a winner, for reasons set
out under "What this project will not claim".

## Pre-registered predictions

Four rungs of increasing cellular resolution:
...
matched unconstrained control set, stratified by LOEUF decile and reported with
confidence intervals. The quantity of interest is the **gap** between the
control curve and the constrained curve.

**If H1 (selection) is right**, the gap stays roughly flat across rungs. Both
curves may rise as power increases, but constrained genes don't catch up.
...
and constrained-gene recovery at rung 4 approaches the control level. That is
the pattern FastGxC reports in blood, where context-specific eGenes' constraint
depletion weakens toward GWAS constraint levels.

**The likely real answer, stated in advance:** partial closure, reported as a
decomposition. "Single-nucleus resolution closes X% of the constrained-gene gap;
the residual Y% is consistent with selection", with a confidence interval on X.

**What would make the result uninterpretable**, also stated in advance: recovery
tracking each study's donor count rather than its resolution. That is the
project's main known risk and the next section deals with it.
...
eQTL-calling pipeline, not only in resolution. Any naive reading of the recovery
curve partly measures power, which is Rosen's point rather than a test of it.
...
2. **Splitting cell classes into their own subtypes** within SingleBrain. Same
   donors, same nuclei, same pipeline; only the grouping changes. Run in Stage 1,
   the gap moves by -0.007 [-0.021, +0.005], which bounds resolution at roughly
   10% of the observed closure.
3. Bryois et al. 2022 enters as a **within-assay power contrast** rather than as
   a rung: single-nucleus like SingleBrain, but at a much smaller donor count.
4. **The Bryois pseudobulk arm.** The February 2023 update to that Zenodo record
   added `pb[1-22].gz`, a "tissue-like" analysis pooling all nuclei per
   individual from the same donors and pipeline as the eight cell-type files.

A limitation surfaced in Stage 1 that belongs here rather than in a footnote:
none of these isolates resolution cleanly, and probably nothing can. In
single-cell data, resolution and per-context power are *intrinsically coupled*.
At fixed sequencing depth you can't resolve more contexts without putting fewer
reads in each. Splitting (2) loses reads per context and pooling (4) gains them,
so the two vary resolution in opposite directions. Agreement between them
brackets the answer without isolating it.

The headline framing is therefore "resolution-plus-power". Public summary
statistics can bound the split between the two; they can't fully resolve it.

## What this project will not claim

- Not a winner-take-all verdict. The accounts are complementary, and the output
  is a decomposition with uncertainty.
- Not a causal claim from colocalization alone.
- Not a claim that any number from a preprint is settled. Rosen et al. 2026 and
  the November 2025 brain/blood single-nucleus preprint haven't completed peer
  review. Anywhere a number from either would be load-bearing is flagged in the
  methods note.
...
| [PsychENCODE](http://resource.psychencode.org/) — Wang et al. 2018, *Science* 362:eaat8464 | rung 2 | citation required, no redistribution grant | **No** |
...
| [PGC3 SCZ GWAS](https://doi.org/10.6084/m9.figshare.19426775) — Trubetskoy et al. 2022, *Nature* 604:502–508 | Stage 2 SMR | CC-BY-4.0 (public figshare deposit) | Yes |
...
FastGxC (*Cell Genomics* 2026) and OneK1K/CIGMA (*Nature* 2026).

Datasets whose licence doesn't permit rehosting go to `data/restricted/`, which
`.gitignore` excludes as a whole directory. Only derived outputs are published
from them, and only where they can't be used to reconstruct the source.
...
- Every download is stamped with its URL, UTC date, byte count and sha256 in
  `logs/downloads.json`. A file whose hash stops matching its record raises
  instead of being silently re-fetched.
- Methodological choices live in `DECISIONS.md`, and machine-readably in
...
| Document | What it is |
|---|---|
| [**Methods note**](docs/methods_note.md) | Short write-up, bioRxiv style: the headline finding and how it was reached |
| [Interview Q&A](docs/interview_qa.md) | Anticipated questions answered from the results, including what went wrong |
...
Still outstanding: deployment to Streamlit Community Cloud, which needs
authorising from the owner's account (see [docs/deploy.md](docs/deploy.md)), and
the HEIDI/coloc sensitivity arms for Stage 2.

## Related work

Third project in one line of work on schizophrenia genetics, and deliberately
the one that asks a question rather than building a ranking.
...
- **[scz-target-prioritization](https://github.com/itsslightning/scz-target-prioritization)**
  — ranks GWAS genes on association, druggability and cell-type specificity at
  once. Establishes *which genes* are worth pursuing, and surfaced CHRM4 at rank
  7 before Cobenfy's approval made the muscarinic mechanism obvious.
- **This repo** asks why the regulatory evidence for those genes is missing in
  the first place, and whether that is biology or measurement.
...
read only what the pipeline already wrote. What's new here is
`pipeline/decisions.py`, which holds conclusion-shaping choices as sentinels that
refuse to default.
