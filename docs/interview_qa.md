...
to [`../DECISIONS.md`](../DECISIONS.md), and every figure is reproducible with
...
Disease genes are the ones eQTL studies are worst at finding regulatory variants
for. Two published accounts disagree about why. Mostafavi says selection removed
the variants; Rosen says we lack the power to see them. Nobody had tested it in
brain, so I built a resolution ladder (bulk cortex, bulk brain, single-nucleus
cell types, subtypes) and measured the gap between constrained genes and matched
controls at each rung.
...
1. **Within bulk tissue, donors do nothing to the gap.** PsychENCODE has 1,387
   donors against GTEx's 205, a factor of 6.8, and the gap is 0.367 in both.
   They agree to three decimal places. Closure across that step is
   **−4.1% [−17.4, +7.7]**.
2. **The winning arm has *fewer* donors.** PsychENCODE → SingleBrain is a 29%
   *decrease* in donors and a **+61.4%** closure.
3. **The smallest study shows the closed gap.** Bryois pseudobulk, at 192
   donors, has a gap of 0.144 against PsychENCODE's 0.367 at seven times the
   donors.
...
Where power *does* matter is absolute counts. Stage 2 loci go 19 → 27 within
bulk for 6.8× donors, because more donors mean more genes clear the instrument
...
it explains the disease. It also wouldn't be new. FastGxC and OneK1K/CIGMA have
done the constrained-gene question in blood already; brain, and SCHEMA
specifically, was the open part.

The honest limitation is that brain is where sample sizes are worst, which is
why the power confound had to be handled properly rather than waved at.
...
With hindsight the ladder is the wrong *shape* for the answer it produced. Rungs
3 and 4 differ only in granularity, which turned out not to matter, while rungs
1–2 versus 3–4 differ in assay, which turned out to be everything. If I rebuilt
it, the primary axis would be assay, with donor count as a covariate and
granularity as a secondary arm.

I also added a rung the report didn't specify. Bryois pseudobulk varies
resolution in the *opposite* direction, pooling rather than splitting, with
donors fixed. That arm did more work than any of the original four.

---

### "What would change your conclusion?"

Four things, in order of how much they'd move me.
...
1. **A bulk brain eQTL study at ~1,000 donors closing the gap as much as
   ...
   claim. Nobody has run it, and it's the obvious next experiment.
2. **A confound tracking assay class across all four datasets.** The assay
   ...
   single-nucleus arms from two consortia agree, so a confound would have to
...
3. **The residual disappearing under a more sensitive uniform rule.** My
   ...
   rule closed the residual 40%, selection would be in trouble.
...
Concretely: study-native thresholds report **88%** closure and my uniform rule
reports **60%**. Anyone quoting "single-nucleus closes ~90% of the gap" from
published eGene counts is partly quoting that artefact.

So I recompute identically everywhere, using per-gene Bonferroni over cis
variants then BH within rung. It's conservative, and absolute counts fall below
every published figure, but it's conservative *by the same construction at every
rung*. That's what a cross-rung comparison needs.
...
expression (−0.61) and *lower* LOEUF (+0.40), so it was discarding the genes the
hypothesis is most about. With replacement retains 94% at better balance (SMD
0.009/0.003) and gives a representative case set. The cost is reused controls,
so they carry frequency weights and intervals are cluster-bootstrapped.
...
Fair question, because the denominator nearly did decide the answer.
...
18,481-gene universe it becomes measurable: 0.445 → 0.579. That's decision
D-012, made before the recovery curve was computed.

There's a related trap. The raw LOEUF-decile view is dominated by expression,
since median cortex TPM falls 12.71 → 0.18 across the deciles, and at
single-nucleus rungs it *inverts*, making the most constrained genes look best
recovered. That figure is in the repo with all three panels, because it's the
clearest demonstration of why matched controls exist.
...
brain at ~1,000 donors should *not* close the gap. That's testable and it would
sink the claim.

I also found it by repairing a broken rung rather than by looking for it.
PsychENCODE was provisional for most of the project because the available file
contained only significant genes. When I got the full 3.3 GB file, the
highest-donor bulk study turned out to have the largest gap, which overturned a
conclusion I'd already committed and written up. The correction is in the git
...
Three things, all in the repo.

1. **I concluded the closure was donor count** and committed it. Rung 2 was
   broken at the time, and it was the one data point that could separate donors
   from assay. Repairing it inverted the conclusion.
2. **I reported the sign of noise.** The resolution test compared point
   ...
   corrected version is a *stronger* claim, a bound of ≤10%, but it was wrong as
   first written.
3. **A figure with a false title.** I labelled the LOEUF-decile figure
   "monotone" when it plainly wasn't. The underlying pattern turned out to be an
   expression confound worth a three-panel figure of its own.
...
No muscarinic receptor reaches SMR significance, and CHRM4 has **zero detectable
cis-eQTLs at any assay or resolution**. But look at what CHRM4 is: LOEUF 0.265,
which puts it in decile 0 and in this study's own constrained case set; pLI
0.974; 11.14 TPM in cortex, so not a low-expression artefact; and an approved
drug target since Cobenfy in 2024.
...
surfaced it. My other repo found it by a route that didn't depend on eQTLs. It
sits in the residual 40% that no assay closes.
...
statistics at every gene, and those exist for only two of five arms. GTEx v10
publishes eGenes only, and SingleBrain's full associations run to about 150 GB,
roughly 64 hours at the throughput I measured, and they aren't range-indexed.
Running coloc only where the data happens to be complete would compare rungs
scored by different methods, which is the exact error D-001 exists to avoid.

SMR needs only the top SNP's effect and standard error, which every rung ships.
The cost is that it can't separate a shared causal variant from linkage, so
counts inflate, but they inflate identically at every rung and the question is a
cross-rung comparison. **These counts shouldn't be quoted against published
coloc figures.** HEIDI and coloc sensitivity arms on the two complete arms are
the next thing I'd run.
...
b_xy = +0.189, and higher expression raising risk is the best-established causal
direction in schizophrenia. CACNA1C, FURIN, KCTD13 and BTN3A2 are all in the top
15. The b_xy sign balance is 0.501, so there's no systematic orientation error.
...
*alternate*, so the conventions are systematically opposite. Because I match on
actual allele letters rather than an assumed convention, the flip rate is
confirmation. Had I got it wrong, every causal estimate would have flipped sign,
turning a risk-increasing gene into an apparently protective one, silently and
plausibly.
