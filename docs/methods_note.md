...
genes while GWAS signal is enriched there, and proposed a **selection** account:
common regulatory variants of large effect are removed at dosage-sensitive
genes, so the missing regulation is real biology and should persist at any
resolution. Rosen *et al.* (2026, AJHG; bioRxiv 2025.08.05.668745) counter with
a **power** account: larger eQTL studies find weaker eQTLs, and those look
progressively more GWAS-like, so the gap is substantially a measurement problem
that should shrink as power and resolution increase.
...
threshold. The analysis would stand unchanged if their reported effect sizes
were revised.
...
`logs/downloads.json`. MetaBrain was dropped: access is gated behind a form and
cannot be scripted.

## Methods

**Gene universe.** One fixed denominator of **18,481** protein-coding genes with
a gnomAD LOEUF value and GTEx brain expression, used for every rung. This is
load-bearing rather than bookkeeping: scored against genes-tested instead, 97%
of genes in SingleBrain excitatory neurons are eGenes, the measure saturates,
and constrained-gene recovery would rise to ≈1 at high resolution *regardless of
biology*.
...
replacement retains 2,767 of 2,937 cases at SMD 0.009/0.003; 1,294 unique
...
study, so q ≤ 0.05 is a *more permissive* bar in a better-powered study, which
would loosen the threshold precisely where power is highest. Detection is
therefore recomputed identically everywhere: per-gene Bonferroni over the cis
variants tested, then Benjamini–Hochberg across all gene × cell-type tests
within a rung (so a rung with 28 cell types pays for its 28 opportunities).
Absolute eGene counts consequently fall below every published figure; that is
the price of comparability and these numbers should not be compared to a
paper's headline eGene count.
...
allele pair — an undetected orientation flip would silently invert the sign of
...
Closure bulk cortex → single-nucleus: **59.9% [49.5, 70.5]**. Across every
structural variant tested — gnomAD v4.1 instead of v2.1.1, pLI instead of
...
**Cell-type resolution is excluded.** Two tests, varying resolution in opposite
directions with donors held fixed. Splitting SingleBrain classes into their own
subtypes: **−0.007 [−0.021, +0.005]** (pooled over 6 classes; every per-class
interval spans zero). Pooling Bryois nuclei into pseudobulk versus its 8 cell
types: **+0.005 [−0.035, +0.048]**. A matched-gene-set control confirms the two
studies do not disagree. Read as a bound: resolution accounts for at most ~10%
of the observed closure.
...
Bulk spread **0.000** across 6.8× donors; single-nucleus spread 0.035 across
5.1×; between-class separation **0.217**. The sharpest comparison is Bryois
pseudobulk — single-nucleus with all nuclei pooled, the smallest study here —
at 0.144 against PsychENCODE bulk at 0.367 with seven times the donors.
...
locating the gain precisely: single-nucleus data does not make eQTLs more
likely to be causal, it makes more genes testable at all.

Positive control: **C4A** is the top-ranked non-MHC gene with b_xy = +0.189 —
higher expression raises risk, the best-established causal direction in
schizophrenia genetics — and the sign balance across significant results is
0.501, indicating no systematic orientation error.

**Targets.** 516 genes are causally implicated only in single-nucleus data; 175
are small-molecule tractable and 24 are at Phase 1 or beyond, including the
calcium-channel family (CACNA1C, CACNA1D, CACNA1I, CACNB2). Tractability rates
are near-identical across visibility groups (~32–34%), so single-nucleus data
is finding more genes at the same druggable rate rather than enriching for
druggable ones.

## Interpretation

Neither hypothesis the project set out to adjudicate survives intact. Selection
predicts the gap persists at any resolution; it does not. Power predicts it
closes with sample size; it does not do that either. The gap is set by **what
is measured** — nuclei or whole tissue.

Candidate mechanisms, stated as hypotheses rather than findings: snRNA-seq
captures largely nascent nuclear transcript while bulk captures mature,
stability-buffered cytoplasmic mRNA, and post-transcriptional buffering of
...
genes. Fractionation data could distinguish these.
...
TPM), is an approved drug target since 2024 — and has no detectable cis-eQTL at
any assay or resolution here. An eQTL-based target-discovery pipeline would
never surface it through regulatory evidence.

## Limitations

Bulk and single-nucleus studies differ in ancestry, brain region, pipeline and
GENCODE vintage as well as assay; the evidence that this is assay rather than a
study-level accident is that two bulk studies agree exactly despite 6.8×
...
across four independent datasets. Resolution and per-context power are
intrinsically coupled in single-cell data, so no test isolates resolution alone;
two tests vary it in opposite directions and bracket it. SMR cannot separate a
shared causal variant from linkage, so Stage 2 counts inflate; HEIDI and coloc
sensitivity arms are not yet run. Locus definition is distance-based, not
LD-based. This is an observational re-analysis: no claim of causality follows
from SMR alone.

## Data and code availability

All analysis code, decision log and derived tables:
https://github.com/itsslightning/missing-regulation-ladder —
`uv sync && uv run python scripts/run_all.py` reproduces every table and figure.
Non-redistributable sources are excluded from the repository; only derived
