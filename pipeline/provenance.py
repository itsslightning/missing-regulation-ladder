...
Every fetch writes one entry to logs/provenance.json holding the URL, the UTC
...
from pipeline.config import PROVENANCE_LOG, ROOT
...
def _load() -> dict:
    if PROVENANCE_LOG.exists():
        return json.loads(PROVENANCE_LOG.read_text(encoding="utf-8"))
    return {}
...
def record(source: Source, path: Path, *, url: str | None = None) -> dict:
    """Stamp a file that is already on disk, and return its provenance entry.
